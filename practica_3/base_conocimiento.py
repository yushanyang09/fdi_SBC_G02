...
def leer_base_conocimiento(base):
    ...
    secciones = [
        seccion.strip()
        for seccion in re.split(r"\s*\.\s*(?=\w+:|$)", texto_filtrado)
        if seccion.strip()
    ]
    ...
    for seccion in secciones:
        ...
        afirmaciones = [
            afirmacion.strip()
            for afirmacion in seccion.split(";")
            if afirmacion.strip()
        ]
...
def add(comando, bc):
    ...
    try:
        ...
        if len(afirmacion[1:]) != 3:
            raise ValueError(
                "La afirmación debe contener exactamente tres elementos (sujeto, predicado y objeto)."
            )
...
def save(comando, afirmaciones):
    ...
    try:
        ...
        if len(archivo_salida) != 2:
            raise ValueError(
                "El formato del comando es erróneo. Asegurate de introducir 'save <archivo>.txt'"
            )
...
def draw(comando, afirmaciones):
    ...
    try:
        ...
        if len(nombre_archivo) != 2:
            raise ValueError(
                "El formato del comando es erróneo. Asegurate de introducir 'draw <archivo>.png'"
            )
